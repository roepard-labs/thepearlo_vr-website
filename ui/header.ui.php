...
<script>
    ...
    (function () {
        ...
        const initialUser = <?php echo $isAuthenticated ? json_encode(['first_name' => $userName, 'role_id' => $userRole]) : 'null'; ?>;

        console.log('📊 Estado inicial PHP (frontend):', { initialAuth, initialUser });
        ...
        function checkBackendSession() {
            console.log('🔍 Verificando sesión del backend (localhost:3000)...');
            
            const apiUrl = window.ENV_CONFIG?.API_URL || 'http://localhost:3000';
            
            // Usar fetch con credentials para compartir cookies
            fetch(apiUrl + '/routes/user/check_session.php', {
                method: 'GET',
                credentials: 'include', // CRÍTICO para CORS con cookies
                headers: {
            ...
            .catch(error => {
                ...
                console.error('💡 Verifica que el backend esté corriendo en localhost:3000');
            });
        }
